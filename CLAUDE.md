...
dependencies and no build. `npm run check` runs them all, `npm test` proves
each one still catches the breaks aimed at it, and GitHub Actions
runs both on every pull request; `main` is protected, so a PR cannot merge
until that run passes. No database or UI exists yet.
